...
var cript = require('./routes/cript');
//---------------------------------------------
...
io_clients.on("connection",function (client) {
  ...
  /* When clent set his position */
  client.on("my-position",function (packet) {

    var db = require("./my_data_base/my_users").users;
    //db[packet.from].position.lat = packet.lat;
    //db[packet.from].position.lon = packet.lon;
    //console.log(db[packet.from].position)
  });
